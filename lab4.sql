...
select count(cus_id) as num_of_customers,cus_gender as gender from customer c;
(select cus_id,ord_amount from order_directory.order
where ord_amount>=3000 group by cus_id) as P
on c.cus_id=P.cus_id group by cus_gender;

select Q.ord_id.pro_name from product pd inner join
(select P.ord_id,pro_id from supplier_pricing s inner join
(select * from new order_directory.order where cus_id=2) as P
where s.pricing_id=P.pricing_id) as @ on pd.pro_id=Q.pro_id;

select product.pro.name, 'order'.* from 'order', supplier_pricing, product
